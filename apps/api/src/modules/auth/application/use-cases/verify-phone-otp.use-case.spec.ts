...
describe(VerifyPhoneOtpUseCase.name, () => {
  ...
  it('throws 401 when flow is missing', async () => {
    ...
    const uc = new VerifyPhoneOtpUseCase(userRepo, flowStore, correlationIdService, configService);
  ...
  it('marks phoneVerified true', async () => {
    ...
    expect(userRepo.update).toHaveBeenCalledWith('u1', { phoneVerified: true });
